...
import { X, MapPin, Calendar, Clock, CheckCircle2, Navigation } from "lucide-react-native";
...
export default function EventDetailModal({ isVisible, onClose, event }: EventDetailModalProps) {
  const { t, lang } = useLanguage();
  const { profile } = useProfile();
  const { isSignedUp, status, signUp, cancel, checkIn } = useRegistration(event?.id, profile?.id);
  ...
  // Improved cross-platform map logic
  const openMap = () => {
    const encodedLocation = encodeURIComponent(event.location);
    
    // Logic: iOS app for iOS, Geo intent for Android, Google Maps URL for Web/PC
    ...
    if (url) {
      Linking.canOpenURL(url).then((supported) => {
        if (supported) {
          ...
          // Fallback for browsers that might struggle with deep links
        ...
        <View className="bg-white rounded-t-[40px] h-[85%] px-6 pt-8 pb-10 shadow-2xl">
          ...
          <View className="flex-row justify-between items-center mb-6">
            <View className="flex-1 pr-4">
              <Text className="text-2xl font-black text-gray-900 leading-tight">{event.title}</Text>
            </View>
          ...
          <View className="pt-4 border-t border-gray-100">
            {!isSignedUp ? (
              <TouchableOpacity onPress={signUp} className="bg-ngoGreen h-16 rounded-2xl flex-row items-center justify-center shadow-lg active:scale-95 transition-all">
